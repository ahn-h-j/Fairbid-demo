...
/**
 * 테스트용 AiClientPort + DescriptionGeneratorPort fake.
 *
 * <p>Cucumber 시나리오에서 모드를 바꿔가며 정상/실패 응답을 시뮬레이션한다.
 * test 클래스패스에 @Primary @Component 로 등록되어 운영용 Claude/Gemini 어댑터를 대체한다.</p>
 */
@Component
@Primary
public class FakeAiClient implements AiClientPort, DescriptionGeneratorPort {
    ...
    public String generateDescription(
        ...
        checkFailure();
        return "테스트용 더미 상품 설명입니다. 이 상품은 매우 좋은 상태이며, 사용감이 거의 없습니다. "
                + "구매 후 박스에 보관하여 외관 상태가 매우 양호합니다. "
                + "빠른 거래를 원하시는 분께 추천드립니다. 자세한 상태는 사진을 참고해주세요.";
    }
