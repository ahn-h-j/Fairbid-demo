...
export default function AuctionDetailPage() {
  ...
  const handlePlaceBid = async (bidType, amount) => {
    ...
    try {
      ...
      if (bidType === BID_TYPES.INSTANT_BUY) {
        mutate(
          (prev) =>
            prev ? { ...prev, status: 'INSTANT_BUY_PENDING', instantBuyEnabled: false } : prev,
          { revalidate: false },
        );
      }
    } catch (err) {
  ...
  const handleDirectBid = (e) => {
    ...
    if (!amount || amount < (auction?.nextMinBidPrice || 0)) {
      setBidMessage({
        type: 'error',
        message: `최소 ${formatPrice(auction?.nextMinBidPrice)} 이상 입력해주세요.`,
      });
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-6 ring-1 ring-black/[0.04]">
        ...
        <h1 className="text-lg sm:text-xl font-bold text-gray-900 leading-snug mb-1.5">
          {auction.title}
        </h1>
        {auction.description ? (
      ...
      {isBidding && user?.userId && (auction.userBidRank === 1 || hasBid || wasFirstRank) && (
        <div
          className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl ring-1 animate-slide-up ${
        ...
        >
          ...
          >
            <span className="text-lg">{auction.userBidRank === 1 ? '🏆' : '📉'}</span>
          </div>
          <div>
            <p
            ...
            >
              {auction.userBidRank === 1 ? '회원님이 현재 1순위입니다' : '1순위가 아닙니다'}
            </p>
            <p
      ...
      {isBidding ? (
        <div className="bg-white rounded-2xl p-5 sm:p-6 ring-1 ring-black/[0.04] space-y-4 animate-slide-up">
          ...
          <p className="text-[11px] text-gray-400 text-center font-medium">
            최소 입찰가 {formatPrice(auction.nextMinBidPrice)} · 입찰 단위{' '}
        ...
        /* 종료된 경매 - 거래 안내 UI */
        <EndedAuctionSection auction={auction} user={user} />
      )}
      {/* 테스트 도구 (관리자 전용) */}
...
function TestTools({ auctionId, mutate }) {
  ...
  return (
    <div className="bg-amber-50/50 rounded-2xl p-5 ring-1 ring-amber-200/60">
      ...
      {testMessage ? (
        <div
          className={`mb-3 px-3 py-2 rounded-lg text-[12px] font-medium ${
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="flex gap-1.5">
          ...
          <button
            type="button"
            onClick={() =>
              callTestApi(`/test/auctions/${auctionId}/set-end-time?seconds=${seconds}`)
            }
            disabled={testLoading}
...
function EndedAuctionSection({ auction, user }) {
    ...
    return (
      <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-2xl p-6 ring-1 ring-red-200/60 animate-fade-in">
        <div className="text-center">
          ...
          <p className="text-[12px] text-red-500 mt-2">경고 1회가 부여되었습니다.</p>
        </div>
      </div>
    );
  }
  ...
  if (
    userWinningRank === 2 &&
    (userWinningStatus === 'PENDING_RESPONSE' || userWinningStatus === 'RESPONDED')
  ) {
    return (
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 ring-1 ring-green-200/60 animate-fade-in">
        <div className="text-center">
    ...
    return (
      <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl p-6 ring-1 ring-amber-200/60 animate-fade-in">
        <div className="text-center">
          ...
          <p className="text-[12px] text-amber-500 mt-2">알림을 확인해주세요!</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-gray-50 rounded-2xl p-6 ring-1 ring-black/[0.04] animate-fade-in">
        <div className="text-center">
          ...
          <p className="text-[13px] text-gray-400 mt-1">이 경매는 유찰되었습니다.</p>
        </div>
      </div>
    );
  }
