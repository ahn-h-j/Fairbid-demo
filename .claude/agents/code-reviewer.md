...
- **Block / Warning은 "수정해야 할 항목 목록"이다.** 발견 자체와 해결책만 적는다.
- **"후속 이슈로 트래킹 권장", "별도 PR로 분리 권장", "PR 본문에 포함" 같은 머지/PR 차원의 결정 문구를 출력에 쓰지 말 것.** 그런 결정은 호출자(`/pr` 스킬, 사용자)가 한다. code-reviewer는 발견만 하고 수정 권한은 호출자에게 넘긴다.
- 항목별 우선순위가 있으면 "Block 필수 / Warning 권장" 같은 일반 톤으로만. 어디서 수정하든(이번 PR / 별도 이슈) 호출자가 결정한다.
